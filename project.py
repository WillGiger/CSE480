...
locks = {} #key is connection and value is the lock
temp_locks = {}
...
class Connection(object):
    ...
    def __init__(self, filename,timeout=0,isolation_level=None):
        ...
        self.transaction = 0
        self.table_locks = {}
        self.mode = 0           #0 is deferred 1 is immediate and 2 is exclusive
        self.copy = Database(filename)
        self.lock = len(locks.keys())   #self.lock is unique to connection
    ...
    def execute(self, statement):
        ...
        def create_table(tokens):
            ...
            if tokens[0] == "IF" and tokens[1] == "NOT" and tokens[2] == "EXISTS":
                tokens = tokens[3:]
                if tokens[0] in _ALL_DATABASES[self.filename].tables:
                    return
        ...
        def drop_table(tokens):
            ...
            if len(tokens) > 1:
                if tokens[0] == "IF" and tokens[1] == "EXISTS":
                    tokens = tokens[2:]
                    table_name = tokens[0]
                    if table_name in _ALL_DATABASES[self.filename].tables:
                        del _ALL_DATABASES[self.filename].tables[table_name]
                    else:
                        return
            else:
                table_name = tokens.pop(0)
                ...
                if self.transaction == 1:
                    for i in locks.keys():
                        if i == self.lock:
                            ...
                            if table_name not in temp_locks[i].keys():
                                continue
                            if temp_locks[i][table_name] == 2 or temp_locks[i][table_name] == 3:
                                raise Exception
                    ...
                    self.copy.insert_into(table_name,
                                              row_contents,
                                              qual_col_names=qual_col_names)    #transaction part
                else:
                    for i in locks.keys():
                        if i == self.lock:
                            continue
                            ...
                            if table_name not in temp_locks[i].keys():
                                continue
                            if temp_locks[i][table_name] == 3 or temp_locks[i][table_name] == 2:
                                raise Exception
        ...
        def update(tokens):
            ...
            if self.transaction == 1:
                ...
                if temp_locks[self.lock][table_name] != 3:
                    temp_locks[self.lock][table_name] = 2
                    locks[self.lock] = 2
                self.copy.update(table_name, update_clauses, where_clause)      #transaction copy update
            else:
                for i in locks.keys():
                    if i == self.lock:
                        continue
            ...
            if self.transaction == 1:
                for i in locks.keys():
                    if i == self.lock:
                        ...
                        if table_name not in temp_locks[i].keys():
                            continue
                        if temp_locks[i][table_name] == 3:
                            raise Exception
                if locks[self.lock] != 2 and locks[self.lock] != 3:
                    locks[self.lock] = 1
                ...
                return self.copy.select(
                    ...
                    is_distinct=is_distinct)                        #transaction locks
            else:
                for i in locks.keys():
                    if i == self.lock:
                        continue
        ...
        def begin(tokens):
            ...
            mode = "DEFERRED" # default mode not used
            ...
            if mode == "IMMEDIATE":
                ...
                for i in self.database.tables:
                    self.table_locks[i] = 2
            elif mode == "EXCLUSIVE":
                for i in locks.keys():
                    if i == self.lock:
                        continue
                    else:
                        if locks[i] == 2 or locks[i] == 3:
                            self.transaction = 0
                            raise Exception
                locks[self.lock] = 3
                for i in self.database.tables:
                    self.table_locks[i] = 3
            else:
                for i in self.database.tables:
                    self.table_locks[i] = 0
                locks[self.lock] = 0



            self.copy.tables = deepcopy(self.database.tables)
        ...
        def commit():
            ...
            for i in locks.keys():
                if i == self.lock:
                    ...
                    for j in temp_locks[i].keys():
                        if temp_locks[i][j] == 1:
                            raise Exception


            self.database.tables = deepcopy(self.copy.tables)
            locks[self.lock] = 0
            for i in temp_locks[self.lock]:
                temp_locks[self.lock][i] = 0
